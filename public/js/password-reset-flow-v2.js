(() => {
  ...
  style.textContent = `
    ...
    .password-reset-result{padding:24px 8px;text-align:center}.password-reset-result .icon{font-size:48px;line-height:1;margin-bottom:14px}
    .password-reset-result h3{margin:0 0 10px;font-size:22px;color:#0f172a}.password-reset-result p{margin:0;color:#475569;font-size:16px;line-height:1.65}
    ...
    .password-request-name{color:#0f172a;font-size:17px;font-weight:900;line-height:1.4}.password-request-meta{margin-top:4px;color:#64748b;font-size:14px;line-height:1.5}
    .password-request-time{margin-top:4px;color:#94a3b8;font-size:12px}.password-request-actions{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin-top:11px}
    ...
    .password-request-actions .approve{border:1px solid #bfdbfe;background:#eff6ff;color:#1d4ed8}.password-request-actions .reject{border:1px solid #fecaca;background:#fff7f7;color:#b91c1c}
    ...
    .password-issued-actions{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin-top:17px}.password-issued-actions button{min-height:44px;border:1px solid #cbd5e1;border-radius:10px;background:#fff;font-weight:900;cursor:pointer}
  ...
  function openResetLayer() {
    ...
    layer.innerHTML = `
      ...
      <main class="password-reset-body"><div class="password-reset-card" data-form>
        <p>가입할 때 등록한 차량번호, 이름, 연락처를 입력해 주세요. 요청이 접수되면 자재센터 관리자가 등록된 연락처로 본인 확인 후 임시 비밀번호를 안내합니다.</p>
    ...
    button.onclick = async () => {
      ...
      if (Object.values(body).some((value) => !value)) {
        errorBox.textContent = '차량번호, 이름, 연락처를 모두 입력해 주세요.';
        return;
      }
      ...
      try {
        ...
        layer.querySelector('[data-form]').innerHTML = `
          <div class="password-reset-result"><div class="icon">${issued ? '🔐' : '✅'}</div>
          <h3>${issued ? '임시 비밀번호 발급 완료' : '발급 요청이 접수되었습니다'}</h3><p>${esc(result.message)}</p></div>`;
      } catch (error) {
  ...
  function issuedModal(data, phone) {
    ...
    backdrop.innerHTML = `
      <div class="password-issued-modal"><h3>임시 비밀번호 발급 완료</h3>
  ...
  async function loadAdminRequests(section) {
    ...
    try {
      ...
      list.innerHTML = requests.length ? requests.map((item) => `
        <article class="password-request-item"><div class="password-request-name">${esc(item.vehicleNumber)} (${esc(item.name)})</div>
          <div class="password-request-meta">${esc(item.phone)}</div>
          ...
          </div></article>`).join('') : '<div class="password-request-empty">대기 중인 비밀번호 발급 요청이 없습니다.</div>';
